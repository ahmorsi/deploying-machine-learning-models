...
from src_model.config.core import DATASET_DIR, TRAINED_MODEL_DIR, config
...
def load_dataset(*, file_name: str) -> pd.DataFrame:
    """Load Dataset
    Load Dataset as Dataframe and apply some Transformations
    """
    dataframe = pd.read_csv(Path(f"{DATASET_DIR}/{file_name}"))
    dataframe = dataframe.replace("?", np.nan)
    dataframe["cabin"] = dataframe["cabin"].apply(_get_first_cabin)
    dataframe["title"] = dataframe["name"].apply(_get_title)
